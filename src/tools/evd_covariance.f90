...
!!   - g=B*f = Kron(B_y,B_x)*f, if f is a column major vectorization (Fortran-prefered ordering).
!! 2) if the first dimension of F corresponds to the y direction (y-leading) then:
!!   - g=B*f = Kron(B_y,B_x)*f with a row major vectorization (C-prefered ordering) of F
!!   - g=B*f = Kron(B_x,B_y)*f, with a column major vectorization (Fortran-prefered ordering) of F.
!! All the 2D routines in this module are based on the formula g=B*f = Kron(B_x,B_y)*f. It is assumed either :
...
MODULE evd_covariance
  ...
  USE general_tools, only: readInfo, readMatrix, writeMatrix
  USE debug_tools
  use kronecker
  use exponential_cov
  use evd_tools
...
!public procedures
  ...
  !> \brief read the size of the EV decomposition of B: size of each singular vector, and number of singular vector
  !! \param [in] UFName file name of the matrix of singular vectors
  !! \param [in] DFName file name of the matrix of singular values
  ...
  !> \brief Writes a covariance matrix under the form of an eigenvalues decomposition
  !! \param [in] UFName file name of the matrix of singular vectors
  ...
  !> \brief Saves the covariance matrix represented by its eigenvalues decomposition
  !! \param [in] C covariance matrix derived type
  ...
  SUBROUTINE apply_evd_square_cov(C, f, g, die)
    ...
    SELECT CASE(C%im_nFac)
      ...
      CASE (2)
        ...
        !call debug (C%Dx(size(C%Dx)-4:),'C%Dx = ', tag=dALLWAYS)
        !call debug (C%Dy(size(C%Dy)-4:),'C%Dy = ', tag=dALLWAYS)
      CASE (default_nFact)!default value
        IF(ll_die)THEN
          CALL stop_program('In apply_evd_square_cov, covariance matrix is not loaded')
        ELSE
          g=f
        END IF
    END SELECT
  ...
  SUBROUTINE apply_evd_square_covb(C, fb, gb, die)
    ...
    SELECT CASE(C%im_nFac)
      ...
      CASE (default_nFact)!default value
        IF(ll_die)THEN
          CALL stop_program('In apply_evd_square_covb, covariance matrix is not loaded')
        ELSE
          fb=fb+gb
          gb=0.0_cp
        END IF
...
!privates procedures
  ...
  !> \brief multiply a vector by the covariance matrix given as a Kronecker product of two covariance Bx and By. Each one being given by its EVD. Best suited for two physical dimensions problems where the covariance factorizes.
  !! param[in] Ux matrix whose columns are the leading eigenvalues vector of the covariance matrix Bx in the direction x, size (nx, kx)
  !! param[in] Dx vector of leading eigenvalues in the direction x, size kx
  !! param[in] Uy matrix whose columns are the leading eigenvalues vector of the covariance matrix By in the direction y, size (ny, ky)
  ...
  !! param[in] U matrix whose columns are the leading eigenvectors of the covariance matrix B, size (n, k)
  ...
  !> \brief multiplies a vector by the inverse of the covariance matrix given as its EVD
  !! param[in] U matrix whose columns are the leading eigenvalues vector of the covariance matrix B, size (n, k)
  !! param[in] D vector of leading eigenvalues, size k
  !! param[in] f input vector of size n (size of the discretization grid)
  !! param[out] g output vector of size n
  !! \details
  ...
  !> \brief multiplies a vector by the square root of the covariance matrix given as its EVD
  !! param[in] U matrix whose columns are the leading eigenvalues vector of the covariance matrix B, size (n, k)
  !! param[in] D vector of leading eigenvalues, size k
  !! param[in] f input vector of size n (size of the discretization grid)
  !! param[out] g output vector of size n
...
!     call debug (Dy05(1:5),'Dy05 = ', tag=dALLWAYS)
  ...
  !> \brief multiplies a vector by the inverse of the square root of the covariance matrix given as its EVD
  !! param[in] U matrix whose columns are the leading eigenvalues vector of the covariance matrix B, size (n, k)
  !! param[in] D vector of leading eigenvalues, size k
  !! param[in] f input vector of size n (size of the discretization grid)
  !! param[out] g output vector of size n
