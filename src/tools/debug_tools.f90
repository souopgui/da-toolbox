...
CONTAINS
  ...
  SUBROUTINE stop_program_with_string(val, description)
    ...
    END IF
    IF ( PRESENT(val) ) THEN
      WRITE(*,*) 'the included message or problematic value is: ', val
    ELSE
    END IF
    WRITE(*,*) '  Stopping ...'
    STOP
...
!print_var routines
  SUBROUTINE print_string(ada_val, description)
    CHARACTER(LEN=*), INTENT(IN) :: ada_val
    CHARACTER(LEN=*), INTENT(IN) :: description
    if(len_trim(description)==0)then
      WRITE(*, FMT=DEBUG_AFORMAT) TRIM(description), TRIM(ada_val)
    else
      WRITE(*, FMT=DEBUG_AFORMAT) TRIM(description)//' ', TRIM(ada_val)
    end if
  END SUBROUTINE print_string

  SUBROUTINE print_integer(id_val, description)
    INTEGER, INTENT(IN) :: id_val
    CHARACTER(LEN=*), INTENT(IN) :: description
    WRITE(*, FMT=TRIM(pretty_dformat(id_val))) trim(description), id_val
  END SUBROUTINE print_integer

  SUBROUTINE print_sp_real(rd_val, description)
    REAL(sp), INTENT(IN) :: rd_val
    CHARACTER(LEN=*), INTENT(IN) :: description

    WRITE(*, FMT=DEBUG_RFORMAT) trim(description), rd_val
  END SUBROUTINE print_sp_real

  SUBROUTINE print_dp_real(rd_val, description)
    REAL(dp), INTENT(IN) :: rd_val
    CHARACTER(LEN=*), INTENT(IN) :: description
  ...
  SUBROUTINE print_logical(ld_val, description)
    LOGICAL, INTENT(IN) :: ld_val
    CHARACTER(LEN=*), INTENT(IN) :: description

    WRITE(*, FMT=DEBUG_LFORMAT) trim(description), ld_val
  ...
  SUBROUTINE print_c_int_array1D(ida_val, description)
    ...
    WRITE(*, FMT=array_format) trim(description), ida_val
  ...
  SUBROUTINE print_sp_real_array1D(rda_val, description)
    ...
    WRITE(*, FMT=array_format) trim(description), rda_val
  ...
  SUBROUTINE print_integer_array2D(ida_val, description)
    ...
    array_format = "(A,"//TRIM( NUM2STR(SIZE(ida_val,2)) )//TRIM(pretty_format( ida_val))//")"
    WRITE(*,*)description
...
!debug routines
  ...
  SUBROUTINE debug_minmax_sp_real_array1D(rda_val, description, tag)
    ...
    IF( debug_flag(tag) )THEN
      CALL print_string('', description)
      WRITE(*, "('   min = ',"//RFORMAT//",'  * max = ',"//RFORMAT//")") MINVAL(rda_val), MAXVAL(rda_val)
    END IF
