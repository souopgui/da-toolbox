...
IMPLICIT NONE
  ...
  !! Meaning for the physical direction i: the first observation point is at index ia_lshifts(i), and the last one does not exceed ia_rshifts(i). No observation point before ia_lshifts(i), and no observation point after ia_rshifts(i). All observation points are in the range [ia_lshifts(i), ia_rshifts(i)]
  ...
  !! \details For now, we have routines available only for regular grid and rectangular physical domain. We also assume that the set of observation points is a subset of the grid point so that each observation can be identified by the indices of the associate grid point. These indices are designated by integer coordinate of the observation.
  !! However, the data structure for observation can be used for any shape of the physical space domain, any type of discretization grid, providing the indexes of observation location on the grid. It can also be easily used with observation points different from the actual grid points.
  !<
...
CONTAINS
  ...
  !! \details This routine is designed for a problem in 1 physical dimension with a state variable represented by a 1D array at each time step. For now, it is assumed that observation are taken at computation time and computation grid points so that there is no need for interpolation. tda_obs is a vector of obs_structure, its size is given by the number of time step where observation are sampled. Each element of this vector contains the observation for a given observation time
  !<
  SUBROUTINE conv_obsgap_elt_1D(tda_obs, rda_state, id_ts)
    ...
    IF( find_conv_obs( tda_obs, id_ts, il_idx ) )THEN
      DO ibi = 1, SIZE(tda_obs(il_idx)%ra_obs)
        ...
        tda_obs(il_idx)%ra_obsgap(ibi) = rda_state( il_ix ) - tda_obs(il_idx)%ra_obs(ibi)
      END DO
    END IF
  ...
  !! \details This routine is designed for a problem in 1 physical dimension with a state variable represented by a 1D array at each time step. For now, it is assumed that observation are taken at computation time and computation grid points so that there is no need for interpolation. tda_obs is a vector of obs_structure, its size is given by the number of time step where observation are sampled. Each element of this vector contains the observation for a given observation time. This routine saves the obsgap so that it is not necessary to keep
  !<
  SUBROUTINE ts_cost_1D(tda_obs, rda_state, id_ts)
    ...
    IF( find_conv_obs( tda_obs, id_ts, il_idx ) )THEN
      DO ibi = 1, SIZE(tda_obs(il_idx)%ra_obs)
        ...
        tda_obs(il_idx)%ra_obsgap(ibi) = rda_state( il_ix ) - tda_obs(il_idx)%ra_obs(ibi)
      END DO
      tda_obs(il_idx)%r_cost = 0.5_cp*SUM( tda_obs(il_idx)%ra_Rm1 * tda_obs(il_idx)%ra_obsgap**2 )
    END IF
  ...
  SUBROUTINE ts_cost_1D_single_obs(td_obs, rda_state)
    ...
    DO ibi = 1, SIZE(td_obs%ra_obs)
      ...
      td_obs%ra_obsgap(ibi) = rda_state( il_ix ) - td_obs%ra_obs(ibi)
    END DO
    td_obs%r_cost = 0.5_cp*SUM( td_obs%ra_Rm1 * td_obs%ra_obsgap**2 )
  ...
  !! \param[in] rda_state state variable
  !! \param[in] id_ts time step associated with the adjoint state variable
  !! \details This routine is designed for a problem in 2 physical dimension with a state variable represented by a 2D array at each time step. For now, it is assumed that observation are taken at computation time and computation grid points so that there is no need for interpolation. tda_obs is a vector of obs_structure, its size is given by the number of time step where observation are sampled. Each element of this vector contains the observation for a given observation time. This routine saves the obsgap so that it is not necessary to keep
  !<
  SUBROUTINE ts_cost_2D(tda_obs, rda_state, id_ts)
    ...
    IF( find_conv_obs( tda_obs, id_ts, il_idx ) )THEN
      DO ibi = 1, SIZE(tda_obs(il_idx)%ra_obs)
        ...
        !the next line of code helps to not passed the state to the associated adjoint routine
        tda_obs(il_idx)%ra_obsgap(ibi) = rda_state( il_ix1, il_ix2 ) - tda_obs(il_idx)%ra_obs(ibi)
      END DO
      tda_obs(il_idx)%r_cost = 0.5_cp*SUM( tda_obs(il_idx)%ra_Rm1 * tda_obs(il_idx)%ra_obsgap**2 )
  ...
  !! \param[in] rda_state state variable
  !! \param[in] id_ts time step associated with the state variable
  !! \details This routine is design for a problem in 2 physical dimensions with a state variable represented by a 2D array at each time step. For now, it is assumed that observation are taken at computation time and computation grid points so that there is no need for interpolation. tda_obs is a vector of obs_structure, its size is given by the number of time step where observation are sampled. Each element of this vector contains the observation for a given observation time
  !<
  SUBROUTINE conv_obsgap_elt_2D(tda_obs, rda_state, id_ts)
    ...
    IF( find_conv_obs( tda_obs, id_ts, il_idx ) )THEN
      DO ibi = 1, SIZE(tda_obs(il_idx)%ra_obs)
        ...
        tda_obs(il_idx)%ra_obsgap(ibi) = rda_state( il_ix, il_iy ) - tda_obs(il_idx)%ra_obs(ibi)
      END DO
    END IF
...
!   end subroutine copy_cop
  ...
  !! \param[in] max (optional) !> max along the physical space, ia_max(i) gives the max index of the observation grid point in the ith dimension
  ...
  !! if present, \a nt does not give the number of observation, it gives the number of time steps at wich observation are authorized. The number of observation takes into account the number of time steps between successive observations. Some values from the input file are ovewritten if the corresponding argument if present: tshift -> il_tshift, nt -> il_tmax,  shifts -> ila_shifts, max -> ila_max. This can be useful in a test case when discretization parameters changed frequently, and that one can forget to update observation parameters in the input file.
  !<
  SUBROUTINE load_cop(td_cop, ada_namelist, ada_varName, ndim, tshift, nt, shifts, max, compute, dt, newVarName)
    ...
    INTEGER, DIMENSION(ndim), OPTIONAL, INTENT(IN) :: shifts, max
...
!       CALL set_obsSize(tm_uobs, il_nobs, id_coord_ndim=il_ndim, ld_icoord=.TRUE., ld_rcoord=.FALSE.)
...
!       tm_uobs%ra_sigma = td_cop%r_u_sigma
!     END IF
...
!---------------------------- Adjoint code section ------------------------------
  ...
  SUBROUTINE ts_costb_1D(tda_obs, rda_stateb, id_ts)
    ...
    IF( find_conv_obs( tda_obs, id_ts, il_idx ) )THEN
      ...
      rla_ogapb = 0.0_cp
      rla_ogapb = rla_ogapb + tda_obs(il_idx)%r_costb * tda_obs(il_idx)%ra_Rm1 * tda_obs(il_idx)%ra_obsgap
