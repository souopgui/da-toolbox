...
!! @version 2.0
!! \details In this version, the mirror boundary condition can
!! be replaced by and extrapolation which is the default.
!! The mirror boundary condition seems to give extra importance
!! to internal points at the boundaries.
!! The removal of the mirror boundary condition do not ensure
!! the theoritical result but, in practice, it produces better results
!! Instead of adding extra-cells to the grid with mirror values,
!! the extra-cells are set to extrapolated values.
!! In 1D, the value of the left extra-cell is given by:
!! f0 = f1 - (1/2f1' + 1/2f2'), this is the value at border cell
!! translated by a linear combination of the derivatives
!! (decentered differences) at the two most external grid cells.
!! Assuming the default 1-based indices in fortran, f0 is the value
!! at the extra-cell. We used the default weight 1/2 for
!! the linear combination. This value can be replaced if necessary
!<
...
CONTAINS
    ...
    !> \brief Ensure boundary conditions on a 1D variable
    ...
    !> \brief Ensure boundary conditions on a 2D variable
    !! \param[out] rda_A array to ensured boundary conditions
    !<
    SUBROUTINE gd_projection_bc_2d(rda_A)
        REAL(KIND=dp), DIMENSION(:, :), INTENT(INOUT)  :: rda_A
        INTEGER                              :: il_dm, il_fm, il_dn, il_fn
        !Make sure that the matrice A of size mxn (m,n > 3) has mirror boundary condition
        il_dm = LBOUND(rda_A,1)
        il_fm = UBOUND(rda_A,1)
