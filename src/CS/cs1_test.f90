...
IMPLICIT NONE
  ...
  !CALL print_ep(tg_ep)
  CALL debug(tg_ep%aa_solver_action, 'Running cs1_test for ')
...
CONTAINS
  ...
  SUBROUTINE direct(rda_ctl, rda_state)
    ...
    CALL fftw_trans(rla_ctl, rda_state)
  ...
  SUBROUTINE adjoint(rda_ctl, rda_ctlb, rda_state, rda_stateb)
    ...
    CALL ifftw_trans(rda_stateb, rla_ctlb)
    !CALL debug(SUM(rda_stateb), 'In adjoint: SUM(rda_stateb) = ')
    !CALL debug(SUM(rla_ctlb_tmp), 'In adjoint: SUM(rla_ctlb_tmp) = ')
...
!     CALL debug(rda_ctl, "CTL = ")
!     CALL dpause()
    CALL debug(rga_state, "STATE = ")
    CALL dpause()
    !CALL debug(200, 'In cost------- ')
    !CALL debug( SHAPE(rga_obsgap), 'In cost, SHAPE(rga_obsgap) = ' )
    !CALL debug( SHAPE(rga_state), 'In cost, SHAPE(rga_state) = ' )
    !CALL debug( SHAPE(tg_obs%ia_icoord), 'In cost, SHAPE(tg_obs%ia_icoord) = ' )
    !CALL debug( SHAPE(tg_obs%ra_obs), 'In cost, SHAPE(tg_obs%ra_obs) = ' )
    rga_obsgap = rga_state( tg_obs%ia_icoord(1,:) ) - tg_obs%ra_obs
    !CALL debug(300, 'In cost------- ')

    rl_ocost = SUM(tg_obs%ra_Rm1*rga_obsgap**2)
    !CALL debug(400, 'In cost------- ')
  ...
  SUBROUTINE make_zero_bg(td_ep)
    ...
    CALL debug('', 'In make_zero_bg: before write_ep_data')
